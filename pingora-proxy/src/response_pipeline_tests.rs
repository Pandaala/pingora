...
use super::*;
use crate::{ResponseHeadHoldLimits, ResponseHeadHoldPlan, ResponseHeadReplacement};
...
impl ProxyHttp for HoldReleaseProxy {
    ...
    fn response_head_commit_plan(
        ...
        Ok(ResponseHeadCommitPlan::Hold(
            ResponseHeadHoldPlan::new_for_test(ResponseHeadHoldLimits::new_for_test(
                1024,
                16,
                32,
                4096,
...
async fn cache_and_custom_hold_are_rejected_before_output() {
    for (protocol, from_cache) in [
        ...
        let error = proxy
            ...
            .await
            .unwrap_err();
        assert!(error.to_string().contains("Hold is unsupported"));
        assert!(out.is_empty());
        assert_eq!(session.prepared_response_headers, 0);
    }
}
