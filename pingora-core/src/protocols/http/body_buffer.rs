...
/// request the client sent empty (registration is rejected for an empty request body;
/// on HTTP/2 the header END_STREAM has already been sent by then). A request whose
/// framing merely *permits* a body but carries zero payload bytes (e.g. chunked
/// terminated by an immediate zero chunk, or an empty END_STREAM DATA frame) does
/// register and may be rewritten to non-empty: while a buffer is registered the
/// session reports the body as non-empty so upstream framing stays open until
/// replay reaches EOF.
///
