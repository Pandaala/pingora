...
`09696b51bc59315353d96686355861604d0bb48c`. The reconstruction froze
`edgion_v3` at `dba39b670b564fdded8af85a1f03f73fd6cb5379` and rebuilt its final
tracked net behavior from the unchanged upstream base as five functional
commits, ending at `3ab9dbe9e12f553d2811b9f20a7849285b9d2012` before the maintenance
knowledge commit. Production code is byte-identical to that frozen v3 tree.
The only non-documentation differences are removal of one extra EOF blank line
from each of two proxy test modules. The untracked review work in the source v3
checkout was not part of the frozen Git tree and was not imported.

Pingora verification on the candidate tree passed:

- formatting, `git diff --check`, the core/proxy check, and the combined
  `connection_filter boringssl` core check;
- default core: 737 passed, 17 ignored; connection-filter core: 742 passed,
  17 ignored; BoringSSL core: 769 passed, 17 ignored; and the focused
  TLS/PROXY target: 2 passed;
...
Consumer contract verification used a clean local clone of Edgion
`5b84915bf4ad19a1c7454cc9e87799cc6cedf92e`. A temporary Cargo patch resolved
all 16 Pingora-family packages to the v4 checkout. The first
workspace/all-target attempt on ExtStore was invalidated by a host-filesystem
failure: Apple's `ranlib` returned `Device not configured`, and concurrent
rustc processes then failed to write fingerprints with SIGBUS. A low-concurrency
retry with a fresh target directory on the internal volume passed
`cargo check -p edgion-gateway --all-targets --offline`, covering the actual
Gateway consumer and its test targets. This is not a successful full-workspace
consumer run. The source Edgion worktree, manifest, and lockfile were not
changed; its original lockfile remained SHA-256
`ecf5d17f37ed9f0d3a84d1b093970971eb70f6f39c3cbd13c9cb96812db77639`.
