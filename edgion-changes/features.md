...
| Layer | Original `edgion_v2` | Migrated `edgion_v3` | Reorganized `edgion_v4` | Purpose |
| --- | --- | --- | --- | --- |
| Inbound transport | `1b33442` | `64d2690` | `d25eb4d` | Parse trusted or mandatory PROXY protocol v1/v2 before TLS |
| Request lifecycle | `879bf3e` | `faf89f5` | `431cd2d` | Capture, finalize, rewind, replay, and carry HTTP body/trailer transport |
| H2 response integrity | `b9ba47c` | `682506d` | `74b1cf3` | Preserve qualified END_STREAM evidence and bound upload non-progress |
| Proxy streaming | `81aa6ac` | `600ac49` | `4bf5ff6` | Add request disposition/termination and async response streaming controls |
| Response-head barrier | — | `af9e1ac` | `3ab9dbe` | Add bounded opt-in Hold with Release/Replace/Fail and protocol-safe abandonment |
| Maintenance knowledge | `2c60acf` | `db3de91` plus later records | current documentation commit | Record behavior, verification, review conclusions, and synchronization policy |

Later `edgion_v3` commits refined these features. `edgion_v4` rebuilds their
final net behavior from the same `09696b5` upstream base as five reviewable
functional commits; tests stay with the functionality they protect. Historical
v2/v3 commits and review records remain provenance, not the current v4 layout.
...
| Response-body streaming controls | Async filter/sink, allocation-free defaults, bounded emitted bytes and chunk count, typed termination, upstream/downstream terminal dispatch, cache/live ordering | trait, shared pipeline, sink, pumps, cache | [response-body-streaming.md](features/response-body-streaming.md) |
...
The bounded response-head commit barrier was introduced in Pingora v3 commit
`af9e1ac`, is reorganized in v4 commit `3ab9dbe`, and was integrated by Edgion
`feature-08-30` at `f31d0169`. It remains default-Immediate unless an
application explicitly opts in. Its closure history and exact verification
snapshot are tracked in
